...
vttcarré = 0 # vartiable utilisé pour la vérifiaction de toute les carrés
g2 = 0
vartampon1 = 0
vartampon2 = 0
vartampon3 = 0
...
# fonction pour verifier que l'entrée de l'utilisateur ne soit pas déja dans la colone
def verif_colonne():
    global colone
...
# vérification que l'utilisatuer mette des valeurs de lignes et de colones comprise entre 1 et 9
def verif_userinput():
    global vu
    if en_ligne > 8:
...
# Vérification que la valeur entrée par l'utilisateur soit comprise entre 1 et 9
def verif_en_utilisateur():
    global veu
    if en_utilisateur >= 10 or en_utilisateur <= 0:
        print("La valeur que vous voulez mettre dans la grille doit étre comprise entre 1 et 9")
        veu = False
...
#fonction utilisé lorsque la grille est totalement rempli pour vérifier toute les lignes
def verif_tt_lignes():
    while vttl != 8:
        if g1[vttl][0] != g1[vttl][1] != g1[vttl][2] != g1[vttl][3] != g1[a][4] != g1[vttl][5] != g1[vttl][6] != g1[vttl][7] != g1[vttl][8]:
            print ("ok ligne :")
            print(vttl)
            print("")
            vttl = vttl + 1
...
#fonction utilisé lorsque la grille est totalement rempli pour vérifier toute les colones
...
#fonction pour vider aléatoirement la grille
...
#fonction pour définir l'entrée de colone du joueur
def entre_de_colone():
    global vartampon2
...
#fonction pour définir l'entrée du joueur
