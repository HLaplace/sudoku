...
#Fonction qui verifie qu'un entier est entre 1 est 9 inclus
...
# Fonction qui vérifie qu'une ligne ne comporte pas deux valeurs identiques
# On passe en paramètre une liste simple qui a 9 colonnes exactement
def verif_valeurs_unique_ligne(my_tab):
    b_valeur_unique = True
    ...
    for i in range(1, 10):
        ...
        for j in range(0, 9):
            if my_tab[j] == i:
                nb = nb + 1
        if nb > 1:
            b_valeur_unique = False
            # print(i) i est la valeur qui n'est pas unique

    return b_valeur_unique

#Fonction qui vérifie que toutes les lignes de mon tableau à 9 lignes et 9 colonnes sont uniques
def verif_valeurs_uniques_toutes_lignes(my_tab):
    b_valeur_unique = True

    for rows in my_tab:
        if not verif_valeurs_unique_ligne(rows):
            b_valeur_unique = False

    return b_valeur_unique


# Fonction qui vérifie qu'une colonne ne comporte pas deux valeurs identiques
# On passe en paramètre un tableau et l'index de la colonne à vérifier
def verif_valeurs_unique_colonne(my_tab,col_index):
    b_valeur_unique = True

    for i in range(1, 10):
        nb = 0
        for j in range(0, 9):
            if my_tab[j][col_index] == i:
                nb = nb + 1
        if nb > 1:
            b_valeur_unique = False
            #print(i) #i est la valeur qui n'est pas unique
...
# Fonction qui vérifie que toutes les colonnes de mon tableau à 9 lignes et 9 colonnes sont uniques
def verif_valeurs_uniques_toutes_colonne(my_tab):
    b_valeur_unique = True

    for i in range(0, 9):
        if not verif_valeurs_unique_colonne(my_tab,i):
            b_valeur_unique = False
...
#avant de terminer le programme, on vérifie que chaque ligne et chaque colonne ne contient qu'une fois une valeur
if verif_valeurs_uniques_toutes_lignes(grille1) and verif_valeurs_uniques_toutes_colonnes(grille1):
    print("Bravo, la grille est correcte !!")
else:
    print("J'ai malheureusement détecté une erreur dans votre grille. Désolé !")
