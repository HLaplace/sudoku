...
#Vérification qu'un entier est bien compris entre 1 et 9
def bool verif_valeur_1_9(int iValeur)
    if iValeur < 1 or i > 9 :
        return True
    else :
        return False

# Vérification que les valeurs de ligne et de colone entrées par l'utilisateur sont
# compatibles avec le jeu soit comprise entre 1 et 9
def verif_userinput():
    global vu
    if en_ligne > 8:
...
# Vérification que la valeur entrée par l'utilisateur est compatible avec le jeu du sudoku
def verif_en_utilisateur():
    global veu
    if en_utilisateur >= 10 or en_utilisateur <= 0:
        print("La valeur que vou voulez mettre dans la grille doit être comprise entre 1 et 9")
        veu = False
